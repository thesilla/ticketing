
<div id = "navbarBottom" class="navbar navbar-expand-lg navbar-dark bg-dark">
    <br/>
    <br/>
    <div id="navbarBottomContent">
        <div class="footer-list">
            <ul>

                <li style = "font-weight: bold;">
                    Wilmington - Location #100

                </li>  
                <br/>
                <li>
                    405 E. Marsh Lane
                </li>  
                <li>
                    Wilmington DE 19804
                </li> 
                <li>
                    302-777-4663
                </li>

                <li>
                    Reception: 100	
                </li> 
                <li>
                    Sales: 188	
                </li> 
                <li>
                    Warehouse: 189	
                </li> 
                <li>
                    Stone Shop Sales Desk: 388	
                </li> 
            </ul>
        </div>

        <div class="footer-list"> 
            <ul>

                <li style = "font-weight: bold;">
                    Lewes - Location #200

                </li>  
                <br/>
                <li>
                    17701 Dartmouth Dr.

                </li>  
                <li>
                    Lewes, DE 19958
                </li> 
                <li>
                    302-644-7100
                </li> 

                <li>
                    Reception: 200
                </li> 
                <li>
                    Sales: 288
                </li> 
                <li>
                    Warehouse: 289	
                </li> 
            </ul>
        </div>
        <div class="footer-list">

            <ul>

                <li style = "font-weight: bold;">
                    Middletown - Location #201

                </li>  
                <br/>
                <li>
                    110 Patriot Dr.

                </li>  
                <li>
                    Middletown, DE 19709
                </li> 
                <li>
                    302-225-9656
                </li> 

                <li>
                    Reception: 400	
                </li> 
                <li>
                    Sales: 488	
                </li> 
                <li>
                    Warehouse: 489	
                </li> 
            </ul>
        </div>

        <div class="footer-list">

            <ul>

                <li style = "font-weight: bold;">
                    Group / Email Address:

                </li>  
                <br/>
                <li>
                    ALL / tmod.all@tilemarketofde.com

                </li>  
                <li>
                    ALL WAREHOUSE / tmod.allwarehouse@tilemarketofde.com
                </li> 
                <li>
                    ALL SALES / tmod.allsales@tilemarketofde.com
                </li> 

                <li>
                    PURCHASING / purchasing@tilemarketofde.com	
                </li> 
                <li>
                    ACCOUNTING / ap@tilede.com	
                </li> 
            </ul>
        </div>





        <br/>




    </div>
    <!--<div style = "color: white; display: block; float: bottom;"> Designed and Developed by Maxwell Gillman. Property of the Tile Market of Delaware 2018 - All Rights Reserved.</div> -->
</div>
